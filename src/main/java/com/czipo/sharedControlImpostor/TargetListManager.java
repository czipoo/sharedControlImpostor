...
public class TargetListManager {
    ...
    public void reload() {
        ...
        plugin.saveResource("list_target.yml", false);
        ...
        // Add blocks that are also items to the items list
        for (String blockName : blocks) {
            try {
                Material mat = Material.valueOf(blockName.toUpperCase(Locale.ROOT));
                if (mat.isItem()) {
                    items.add(blockName);
                }
            } catch (IllegalArgumentException e) {
                // Block doesn't exist as a material, skip
            }
        }
    }
